...
<head>
    <meta charset="UTF-8">
    <title>All Leader Positions Pdf</title>
</head>
<body>
<div style="text-align: center; font-size: 40px; font-weight: bold;">Nafasi Zote</div>
...
<table width="100%" cellpadding="5" cellspacing="0" style="border-collapse: collapse; border: 1px solid #000;">
    ...
    <tr style="background-color: #000; color: #fff; padding: 10px 2px;">
        ...
        <th style="border: 1px solid #000;">Jina la Nafasi</th>
    ...
    @foreach($positions as $index => $position)
        <tr style="background-color: {{ $index % 2 === 0 ? '#f2f2f2' : '#fff' }}; padding: 10px 2px;">
            ...
            <td style="border: 1px solid #000;">{{ $position->name }}</td>
            <td style="border: 1px solid #000;">{{ $position->user->name .'('. $position->user->user_type .')' }}</td>
            <td style="border: 1px solid #000;">{{ $position->created_at }}</td>
        </tr>
    @endforeach
    </tbody>
</table>
